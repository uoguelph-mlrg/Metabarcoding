...
class LatentSolver:
    ...
    Scalar mode (embed_dim == 1):
        min_D (1/2) sum_{s,b} (y_{s,b} - m(s,b) - (H_data D)_b)^2 + (r/2)||D||^2 + (λ/2)||(I-H_smooth)D||^2
        The interpolated latent (H_data @ D)[bin] is used instead of the raw D[bin], where
        H_data is self-inclusive ((I + H_smooth) row-normalized) in interpolated mode.
    ...
    Vector mode (embed_dim > 1):
        ...
        where ŷ = sigmoid(w^T (m(x) ⊙ g((H_data H)[bin])))
        The interpolated latent (H_data @ H)[bin] is used instead of the raw H[bin], where
        H_data is self-inclusive ((I + H_smooth) row-normalized) in interpolated mode.
        Solution via L-BFGS.
    ...
    def __init__(self, cfg: Config, neighbour_graph: NeighbourGraph, embed_dim: int = 1, gating_fn: GatingFnName = "exp"):
        ...
        # self-inclusive interpolation matrix (I + H_smooth, row-normalized) for data term
        self.H_with_self: Optional[sparse.csr_matrix] = None  # (I + H_smooth) normalized per-row
    ...
    def build_interpolation_matrix(
        ...
        # Build self-inclusive matrix: (I + H_smooth) with row normalization
        H_with_self = I + H_smooth
        # Row-normalize: each row sum to 1
        row_sums = np.array(H_with_self.sum(axis=1)).flatten()
        row_sums = np.clip(row_sums, 1e-12, None)  # avoid division by zero
        row_inv = 1.0 / row_sums
        row_inv_diag = sparse.diags(row_inv, format="csr")
        self.H_with_self = row_inv_diag @ H_with_self

        if self.embed_dim == 1:
            # Step 3 (scalar only): Precompute A = (VH)^T(VH) + r*I + λ*(I-H_smooth)^T(I-H_smooth)
            # VH = V @ H_with_self so that interpolated data terms include self latent.
            VH = sparse.csr_matrix(V @ self.H_with_self)
            self.VH = VH
            VHtVH = (VH.T @ VH).tocsc()
    ...
    def _build_latent_mixing_matrix(
        ...
        - normal mode:        M = I
        - interpolated mode:  M = (I + H_smooth) row-normalized (self-inclusive neighbors)
        - mixed mode:         M[b,:] = H_with_self[b,:] if mask[b] else e_b^T
        """
        if self.H_smooth is None:
            raise RuntimeError("H_smooth not built; call build_interpolation_matrix first")
        if self.H_with_self is None:
            raise RuntimeError("H_with_self not built; call build_interpolation_matrix first")

        if interpolated_bin_mask is None:
            if use_interpolated_latent:
                return self.H_with_self
        ...
        return (raw_diag + interp_diag @ self.H_with_self).tocsr()
    ...
    def _solve_logistic(
        ...
        Uses interpolated latent (H_data @ D)[bin] matching the forward pass.

        System: ((VM)^T(VM) + rI + λ(I-H_smooth)^T(I-H_smooth)) D = (VM)^T (logit(y) - m)
        where M is the selected data-term mixing matrix.
        ...
        if self.cfg.latent_present_only:
            ...
            if obs_mask_filtered is not None:
                VH_filtered = sparse.csr_matrix(V_filtered @ self.H_with_self)
            ...
            if obs_mask is not None:
                VH_all = self.VH if self.VH is not None else sparse.csr_matrix(self.V @ self.H_with_self)
    ...
    def _solve_cross_entropy_lbfgs(
        ...
        We minimize:
            sum_s CE(y_s, softmax(m_s + (H_data D)_bins)) + (r/2)||D||^2 + (λ/2)||(I-H)D||^2

        Gradient chain rule: dL_data/dD = H_data^T @ (dL_data / d(H_data D))
        """
        ...
        interp_matrix = self.H_with_self
        assert interp_matrix is not None
        mixing_matrix = self._build_latent_mixing_matrix(use_interpolated_latent, interpolated_bin_mask)
        ...
        def fun_and_jac(D_flat: np.ndarray) -> tuple[float, np.ndarray]:
            ...
            if obs_s is None:
                ...
                D_interp = interp_matrix.dot(D_flat)
            ...
            if obs_s is None:
                ...
                grad_bins = grad_D_raw + interp_matrix.T.dot(grad_D_interp)
    ...
    def _solve_logistic_vector(
        ...
        Uses interpolated latent (H_data @ H)[bin] matching the forward pass.

        Model: ŷ_i = sigmoid(w^T (m_i ⊙ g((H_data H)[bin_i])))
        Objective: BCE(y, ŷ) + (r/2)||H||^2 + (λ/2)||(I-H_smooth)H||^2
        Gradient chain rule: dL_data/dH = H_data^T @ (dL_data / d(H_data H))
        """
        ...
        def fun_and_jac(H_flat):
            ...
            if obs_use is None:
                ...
                grad_H = grad_H_raw + interp_matrix.T.dot(grad_H_interp)
    ...
    def _solve_cross_entropy_lbfgs_vector(
        ...
        Model: p_i = softmax(w^T (m_i ⊙ g((H_data H)[bin_i])))_within_sample
