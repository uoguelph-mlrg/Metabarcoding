#!/usr/bin/env python
"""
Visualization module for loss function comparison results.
Creates clean, presentation-ready plots comparing Cross-Entropy vs Logistic (BCE) loss.

This script is separate from loss_comparison.py to allow re-generating visualizations
without re-running the expensive training.

Usage:
    python loss_comparison_visualize.py --results_path results/loss_comparison_results.pkl --output_dir figures
"""
...
# Loss type colors (using distinct colors from ablation study)
LOSS_COLORS = {
    "cross_entropy": "#2ecc71",   # Green
    "logistic": "#9b59b6",        # Purple
}
...
def plot_metrics_comparison(results: Dict[str, Any], output_dir: str):
    ...
    plt.suptitle('Loss Function Performance Comparison', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def plot_mae_per_range(results: Dict[str, Any], output_dir: str):
    ...
    ax.legend(title='Loss Function', frameon=False, loc='upper left')
    ax.set_xticks(x)
...
def plot_summary_table(results: Dict[str, Any], output_dir: str):
    ...
    data = []
    for lt in loss_types:
        row = {'Loss Function': get_loss_label(lt)}
        for m in metrics:
    ...
    # Color first column (Loss Function) with each model's color
    lf_col_idx = list(display_df.columns).index('Loss Function')
    for row_idx, lt in enumerate(loss_types):
        cell = table[(row_idx + 1, lf_col_idx)]
    ...
    plt.title('Loss Function Comparison Summary', fontweight='bold', fontsize=14, y=0.98)
    plt.tight_layout()
    ...
    # Also save as CSV
    df.to_csv(os.path.join(output_dir, "loss_comparison_results.csv"), index=False)
    log.info(f"  ✓ Saved: loss_comparison_results.csv")
...
def create_all_visualizations(results: Dict[str, Any], output_dir: str):
    ...
    log.info("   - loss_comparison_results.csv")


def print_comparison(results: Dict[str, Any]):
    """Print comparison table to console."""
    log.info("\n" + "="*80)
    log.info("LOSS FUNCTION COMPARISON RESULTS")
    log.info("="*80)
    ...
    log.info(f"Recommended loss function: {get_loss_label(overall_winner)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize Loss Function Comparison Results")
    parser.add_argument("--results_path", type=str, default="results/loss_comparison_results.pkl",
